...
void TAMC_GT911::reset() {
  if (pinInt != 254){ pinMode(pinInt, OUTPUT);}
  if (pinRst != 254){pinMode(pinRst, OUTPUT);}
  if (pinInt != 254){digitalWrite(pinInt, 0);}
  if (pinRst != 254){digitalWrite(pinRst, 0);}
  delay(10);
  if (pinInt != 254){digitalWrite(pinInt, addr==GT911_ADDR2);}
  delay(1);
  if (pinRst != 254){digitalWrite(pinRst, 1);}
  delay(5);
  if (pinInt != 254){digitalWrite(pinInt, 0);}
  delay(50);
  if (pinInt != 254){pinMode(pinInt, INPUT);}
  // attachInterrupt(pinInt, TAMC_GT911::onInterrupt, RISING);
  delay(50);
   // Probe GT911 before reading config
 ...
 // temporary ? readBlockData(configBuf, GT911_CONFIG_START, GT911_CONFIG_SIZE);
 //  setResolution(width, height);
}
...
void TAMC_GT911::readBlockData(uint8_t *buf, uint16_t reg, uint8_t size) {
  ...
  if (received != size) {
   // DEBUG_PORT.printf("GT911 I2C read mismatch: got %d, expected %d\n", received, size);
    ...
    // Fill remaining bytes with 0xFF or leave untouched
    for (uint8_t i = received; i < size; i++) {
      buf[i] = 0xFF;  // or buf[i] = 0;
    }
    return;
  }
